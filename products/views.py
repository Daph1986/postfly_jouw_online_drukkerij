...
def add_product(request):
    ...
    if not request.user.is_superuser:
        messages.error(
            request, 'Sorry, your are not allowed to add a product, that is \
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, 'Success, the product was added to the store!')
            return redirect(reverse('products'))
        else:
            messages.error(
                request, 'Sorry, your attempt to add the product failed, \
...
def update_product(request, product_id):
    ...
    if not request.user.is_superuser:
        messages.error(
            request, 'Sorry, your are not allowed to update a product, that is \
            for store owners only.')
        return redirect(reverse('home'))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(
                request, 'Sorry, your attempt to update the product failed, \
                please check your form!')
    else:
...
def delete_product(request, product_id):
    ...
    if not request.user.is_superuser:
        messages.error(
            request, 'Sorry, your are not allowed to delete a product, that \
            is for store owners only.')
        return redirect(reverse('home'))
